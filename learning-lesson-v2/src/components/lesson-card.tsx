...
export function LessonCard({ lesson, completed, unlocked, labels }: LessonCardProps) {
  return (
    <article className="rounded-lg border border-ink/10 bg-white/75 p-4 shadow-sm">
      ...
      <div className="mt-4 flex items-center justify-between gap-3">
        <span className="text-sm font-semibold text-ink/70">
          {completed ? labels.completed : unlocked ? labels.unlocked : labels.locked}
        </span>
        {unlocked ? (
          <Link
