...
const gradeLabelsBg = { 8: "VIII", 9: "IX", 10: "X", 11: "XI", 12: "XII" } as const;

type GradeRoadmapProps = {
  language: Language;
};

export function GradeRoadmap({ language }: GradeRoadmapProps) {
  ...
  return (
    <div className="mt-7 border-t border-white/10 pt-5">
      <div className="flex items-center justify-between gap-4">
        ...
        <p className="text-xs font-bold text-mint">{copy.activeGrade}: VIII</p>
      </div>
      <ol className="mt-3 grid min-w-[560px] grid-cols-5 gap-2" aria-label={copy.gradeRoadmap}>
        {gradeLevels.map((grade) => {
          const isActive = grade === 8;
          return (
            <li
            ...
            >
              ...
              <span>{language === "bg" ? gradeLabelsBg[grade] : `G${grade}`}</span>
